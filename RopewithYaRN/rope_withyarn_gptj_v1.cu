...
template<>
__global__ __launch_bounds__(512) void gptj_style::rope_withyarn_gptj_v1
    <32, 16, 512>(
...
)
{
    ...
    pos_cs.seq_len_pos = positions[index.blk_x];
    ...
    __device__ void SM_bind() noexcept {
        this->sm_cos = SM_cos;
        this->sm_sin = SM_sin;
        this->sm_q = SM_q;              //[WARP_NUMS][WARP_SIZE]
        this->sm_q_rot = SM_q_rotate;   //[WARP_NUMS][WARP_SIZE]
        this->sm_k = SM_k;              //[WARP_SIZE]
        this->sm_k_rot = SM_k_rotate;   //[WARP_SIZE]
    }
    ...
    //block迭代次数
    struct iters {
    __device__ iters() noexcept = default;
    ...
    if (mask_wh.mask_w == true && mask_sf_load == true) {
        SM_load.sm_cos[sm_c * 2 + 0] = pos_cs.cos_cache[d * WARP_Shfl + index.lane_id()];
        SM_load.sm_sin[sm_c * 2 + 0] = pos_cs.sin_cache[d * WARP_Shfl + index.lane_id()];
    }
    ...
    //gptj风格排列三角缓存
    if (mask_wh.mask_w == true && mask_sf_load == true) {
        SM_load.sm_cos[sm_c * 2 + 1] = SM_load.sm_cos[sm_c * 2 + 0];
        SM_load.sm_sin[sm_c * 2 + 1] = SM_load.sm_sin[sm_c * 2 + 0];
    }

    //gptj风格旋转掩码
    bool mask_gptj = false;
    mask_gptj = (index.lane_id() % 2 == 0)? true:false;

    if (first_h == true && mask_wh.mask_w == true) {
        half k_ = SM_load.sm_k[sm_c];        
        k_ = (mask_gptj == true)? k_:__hneg(k_);
        k_ = __shfl_xor_sync(MASK, k_, 1);
        SM_load.sm_k_rot[sm_c] = k_;
    }

    half q_ = SM_load.sm_q[sm_r][sm_c];
    q_ = (mask_gptj == true)? q_:__hneg(q_);
    q_ = __shfl_xor_sync(MASK, q_, 1);
...
extern "C" __host__ void gptj_style::rope_withyarn_gptj_v1_launch(
...
) noexcept
{
    ...
    CUDA_CHECK(cudaLaunchKernel(
        (void*)&rope_withyarn_gptj_v1<BLOCK_X, BLOCK_Y, BLOCK_Y*BLOCK_X>,
        grid, block,
