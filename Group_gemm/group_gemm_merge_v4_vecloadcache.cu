...
template<>
__global__ __launch_bounds__(256) void group_gemm_merge_v4_vecloadcache
    <16, 16, 2, 2>(
...
)
{
    ...
    __shared__ half sm_A_i[Cache][BLOCK_Y * Threads][K_len];
    __shared__ half sm_B_i[Cache][K_len][BLOCK_X * Threads];
    ...
    #pragma unroll
    for (int dy=0; dy<Threads; dy++) {
        _a[dy]=sm_A_i[curr][blo_y * Threads + dy][k];
        
        for (int dx=0; dx<Threads; dx++) {
            _b[dx]=sm_B_i[curr][k][blo_x * Threads + dx];
...
) noexcept
{
    ...
    CUDA_CHECK(cudaLaunchKernel(
        (void*)group_gemm_merge_v4_vecloadcache<BLOCK_X, BLOCK_Y, Threads, Vec_len>, 
        grid, block, 
        args, 0 , cu_str
    ));
