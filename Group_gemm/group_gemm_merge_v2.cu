...
template<>
__global__ __launch_bounds__(256) void group_gemm_merge_v2
    <16, 16, 2>(
...
)
{
    ...
    for (int dx=0; dx<Threads; dx++) {
        ...
        if (c_row < M_cur && c_col < N_cur) {
            ...
            float val = __fmul_rn(sum_i[dy][dx], alpha) + __fmul_rn(source, beta);
...
) noexcept
{
    ...
    CUDA_CHECK(cudaLaunchKernel(
        (void*)group_gemm_merge_v2<BLOCK_X, BLOCK_Y, Threads>, 
        grid, block, 
        args, 0 , cu_str
    ));
