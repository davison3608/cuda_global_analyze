...
template<>
__global__ __launch_bounds__(256) void group_gemm_merge_v3_vecload
    <16, 16, 2, 2>(
...
)
{
    ...
    __shared__ half sm_A_i[BLOCK_Y * Threads][K_len];
    __shared__ half sm_B_i[K_len][BLOCK_X * Threads];

    //原精度累加
    float sum_i[Threads][Threads] {0.0f};
    ...
    for (int dy=0; dy<Threads; dy++) {
        ...
        sm_A_i[sm_a_row][sm_a_col + 0] = vec_a.x;
        sm_A_i[sm_a_row][sm_a_col + 1] = vec_a.y;
    }
    }
    ...
    for (int dx=0; dx<Threads; dx++) {
        ...
        half vec_b_y = (mask_cur)? B_cur[(b_row_base + k * Vec_len + 1) * N_cur + b_col]:
            __int2half_rn(0);
        sm_B_i[sm_b_row + 0][sm_b_col] = vec_b_x;
        sm_B_i[sm_b_row + 1][sm_b_col] = vec_b_y;
    }
    }
    ...
    half _a[Threads] {0.0f};
    half _b[Threads] {0.0f};
    ...
    #pragma unroll
    for (int dy=0; dy<Threads; dy++) {
        _a[dy]=sm_A_i[blo_y * Threads + dy][k];
        
        for (int dx=0; dx<Threads; dx++) {
            _b[dx]=sm_B_i[k][blo_x * Threads + dx];

            half val = __hmul_rn(_a[dy], _b[dx]);
            sum_i[dy][dx] += __half2float(val);
        }
    }
    }
    ...
    for (int dy=0; dy<Threads; dy++) {
        ...
        float2 val = make_float2(
            __fmul_rn(sum_i[dy][0], alpha) + __fmul_rn(source.x, beta),
            __fmul_rn(sum_i[dy][1], alpha) + __fmul_rn(source.y, beta)
        );
        if (mask_cur)
...
) noexcept
{
    ...
    CUDA_CHECK(cudaLaunchKernel(
        (void*)group_gemm_merge_v3_vecload<BLOCK_X, BLOCK_Y, Threads, Vec_len>, 
        grid, block, 
        args, 0 , cu_str
    ));
