...
template<>
__global__ __launch_bounds__(256) void GroupedTopkfused_fp16_v1
    <32, 8, 256>(
    const half* __restrict__ gating_x,      //[N_tokens, n_routed_experts]
...
)
{
    ...
    public:
    //sigmoid处理原始值
    half original_scores {__float2half(-INFINITY)};
    //叠加偏置的值
    ...
    __device__ __forceinline__ void isNanandin() {        
        this->mask_Nan = __hisnan(this->original_scores);
    ...
    //sigmoid计算 同时添加偏置
    __device__ __forceinline__ void sigmoid(half&val, float&b) {
        ...
        b = __fadd_rn(b, __half2float(val));
    }
    //提交任务 忽略Nan
    ...
    __device__ __forceinline__ void thread_swap (
        float&val, int&id, float&or_val, int&or_id, half&hval, half&or_hval
    ) {
        bool mask_swap = (or_val > val || (or_val == val && or_id < id));
        val = mask_swap? or_val:val;
        id = mask_swap? or_id:id;
        hval = mask_swap? or_hval:hval;
    }
    __device__ __forceinline__ void thread_swap (
    ...
    __shared__ alignas(4) half SM_group_weights_gather[WARP_NUMS * MAX_TOPK_NUMS];
    ...
    __shared__ alignas(4) half SM_sin_group_weights[MAX_TOPK_NUMS];
    ...
    half* sm_grp_w_g = nullptr;  //[WARP_NUMS * MAX_TOPK_NUMS]
    int* sm_grp_i = nullptr;     //[WARP_NUMS * MAX_TOPK_NUMS]
    float* sm_sin_wb = nullptr;  //[MAX_TOPK_NUMS] 
    half* sm_sin_w = nullptr;    //[MAX_TOPK_NUMS]
    ...
    this->sm_grp_w_g[index.warp_id() * MAX_TOPK_NUMS + index.lane_id()] = __float2half(-INFINITY);
    this->sm_grp_i[index.warp_id() * MAX_TOPK_NUMS + index.lane_id()] = -1;
    if (index.warp_id() == 0) {
        this->sm_sin_wb[index.lane_id()] = -INFINITY;
        this->sm_sin_w[index.lane_id()] = __float2half(-INFINITY);
    ...
    half val_scores {__float2half(-INFINITY)};
    ...
    half thread_gather = val_scores;
    int thread_idx = idx_original;

    float *sm_vals = SM_load.sm_grp_w;
    half* sm_gathers = SM_load.sm_grp_w_g;
    int *sm_ids = SM_load.sm_grp_i;

    #pragma unroll
    for (int k=0; k<topk_num; k++) {
    float max_val = thread_val;
    half max_hval = thread_gather;
    int max_idx = thread_idx;
    
    #pragma unroll
    for (int offset_shfl=WARP_Shfl; offset_shfl>0; offset_shfl /= 2) {
        float or_max_val = __shfl_xor_sync(MASK, max_val, offset_shfl);
        half or_max_hval = __shfl_xor_sync(MASK, max_hval, offset_shfl);
        int or_max_idx = __shfl_xor_sync(MASK, max_idx, offset_shfl);
        tswap.thread_swap(
            max_val, max_idx, 
            or_max_val, or_max_idx,
            max_hval, or_max_hval
        );
    }
    ...
    //所有线程持有当前局部最大值 冗余广播保证稳定
    max_val = __shfl_sync(MASK, max_val, 0);
    max_hval = __shfl_sync(MASK, max_hval, 0);
    max_idx = __shfl_sync(MASK, max_idx, 0);

    if (index.lane_id() == 0) {
        sm_vals[index.warp_id() * MAX_TOPK_NUMS + k] = max_val;
        sm_gathers[index.warp_id() * MAX_TOPK_NUMS + k] = max_hval;
    ...
    half hval_len[Vec_len] {__float2half(-INFINITY)};
    ...
    for (int l=0; l<Vec_len; l++) {
        ...
        hval_len[l] = sm_gathers[index.lane_id() * Vec_len + l];
    ...
    half thread_hmax = __float2half(-INFINITY);
    int thread_idx = -1;
    ...
    for (int t=0; t<Vec_len; t++) {
        ...
        thread_hmax = ismax? hval_len[t]:thread_hmax;
    ...
    for (int offset_shfl=WARP_Shfl; offset_shfl>0; offset_shfl /= 2) {
        ...
        half or_hmax = __shfl_xor_sync(MASK, thread_hmax, offset_shfl);
    ...
    thread_hmax = __shfl_sync(MASK, thread_hmax, 0);
    ...
    __shared__ alignas(2) half SM_weights_all;
    half _weights {__int2half_rn(0)};
    ...
    half* top_results = SM_load.sm_sin_w;
    int* top_idx = SM_load.sm_sin_i;

    _weights = mask_toplen? top_results[index.lane_id()]:_weights;
    _weights = __hadd_rn(_weights, __shfl_xor_sync(MASK, _weights, WARP_Shfl));
    _weights = __hadd_rn(_weights, __shfl_xor_sync(MASK, _weights, Shfl_1));
    _weights = __hadd_rn(_weights, __shfl_xor_sync(MASK, _weights, Shfl_2));
    _weights = __hadd_rn(_weights, __shfl_xor_sync(MASK, _weights, Shfl_3));
    _weights = __hadd_rn(_weights, __shfl_xor_sync(MASK, _weights, Shfl_4));
    ...
    _weights = mask_toplen? top_results[index.lane_id()]:_weights;
    _weights = mask_toplen? __hdiv(_weights, SM_weights_all):_weights;
    _fweights = (mask_toplen == true && is_scale == true)? 
        __half2float(__hmul_rn(_weights, __float2half(route_scale))):
        __half2float(_weights);
