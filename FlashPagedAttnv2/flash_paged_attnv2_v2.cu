...
__global__ __launch_bounds__(512)  void decode::flash_paged_attnv2_v2(
...
)
{
    ...
    __device__ SM_view() noexcept {}
        ...
        int _cur, _net {0, 1};
        float alignas(4) sm_q[Cache][BLOCK_H][WARP_SZ];
        float alignas(4) sm_k[Cache][BLOCK_N][WARP_SZ];
        float alignas(4) sm_v[Cache][BLOCK_N][WARP_SZ];
    ...
    if (index.warp_id() < BLOCK_N) {
        ...
        this->sm_k[c][index.warp_id()][index.lane_id()] = 0.0f;
        this->sm_v[c][index.warp_id()][index.lane_id()] = 0.0f;
        if (index.warp_id() < BLOCK_H)
            this->sm_q[c][index.warp_id()][index.lane_id()] = 0.0f;
        }
    }
    else
        return;
    ...
    __device__ __forceinline__ void change_step() noexcept { 
        auto tmp = this->_cur; 
        this->_cur = this->_net;  
        this->_net = tmp;
    }
    __device__ __forceinline__ void reset_step() noexcept {
        this->_cur = 0;
        this->_net = 1;
    }
    __device__ __forceinline__ int&cur_step() noexcept 
    { return this->_cur; }
    __device__ __forceinline__ int&next_step() noexcept 
    { return this->_net; }
    };
    ...
    //原始注意力矩阵阶段 预加载一次从而异步计算与缓存
    for (int dk=0; dk<1; dk++) {
        int offset_dk = dk * WARP_SZ + index.lane_id();
        SM_load.sm_k[SM_load.cur_step()][index.warp_id()][index.lane_id()] = (is_effect == true)? 
            K_pagtoken[offset_dk]:-INFINITY;

        if (index.warp_id() == 0)
        SM_load.sm_q[SM_load.cur_step()][index.blo_z][index.lane_id()] = Query[
    ...
    for (int dk=1; dk<iter_dk; dk++) {
        float qk_t = __fmul_rn(
            SM_load.sm_k[SM_load.cur_step()][index.warp_id()][index.lane_id()], 
            SM_load.sm_q[SM_load.cur_step()][index.blo_z][index.lane_id()];
        );
        ...
        if (dk < iter_dk - 1) {
        int offset_dk = dk * WARP_SZ + index.lane_id();
        SM_load.sm_k[SM_load.next_step()][index.warp_id()][index.lane_id()] = (is_effect == true)? 
            K_pagtoken[offset_dk]:-INFINITY;

        if (index.warp_id() == 0)
        SM_load.sm_q[SM_load.next_step()][index.blo_z][index.lane_id()] = Query[
            0 * num_q_heads * qk_head_dim + offset_head_glo * qk_head_dim +
            offset_dk
        ];
        }
    ...
    for (int dv=0; dv<1; dv++) {
        ...
        SM_load.sm_v[index.cur_step()][index.warp_id()][index.lane_id()] = (is_effect == true)? 
            V_pagtoken[offset_dv]:-INFINITY;

        SM_load.syncblock();
    }
    for (int dv=1; dv<iter_dv; dv++) {
        if (index.warp_id() < BLOCK_H) {
        float val_sum = 0.0f;
        ...
        for (int i=0; i<BLOCK_N; i++) {
            ...
            float dv_t = __fmul_rn(
                ...
                SM_load.sm_v[SM_load.cur_step()][i][index.lane_id()]
            )
            val_sum += dv_t;
        }
        ...
        int offset_dv = dv * WARP_SZ + index.lane_id();
        //sm_p[BLOCK_H, BLOCK_N] * sm_v[BLOCK_N, WARP_SZ] = acc_dv[BLOCK_H, WARP_SZ]
        SM_load.sm_v[index.next_step()][index.warp_id()][index.lane_id()] = (is_effect == true)? 
            V_pagtoken[offset_dv]:-INFINITY;
        }
